...
const ResultsTable: React.FC<ResultsTableProps> = ({ result, theme }) => {
    ...
    return (
        <div className="flex-1 overflow-hidden bg-terminal-bg flex flex-col transition-colors relative border-t border-terminal-border/50 min-h-[100px]">
            {/* Grid Header */}
            <div className="px-4 py-2 bg-terminal-surface/80 dark:bg-terminal-surface/80 backdrop-blur-sm border-b border-terminal-border flex justify-between items-center sticky top-0 z-20 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <span className="px-2 py-0.5 bg-terminal-blue/10 border border-terminal-blue/20 rounded text-[10px] font-mono text-terminal-blue font-bold">
                        {result.data.length} ROWS
                    </span>
                    
                    {/* View Toggle */}
                    <div className="flex bg-white dark:bg-terminal-bg rounded border border-terminal-border/50 p-0.5 ml-4 shadow-sm">
                        <button 
                ...
                <span className="text-[10px] font-mono text-terminal-text/40 bg-white dark:bg-black/20 px-2 py-1 rounded border border-terminal-border/30">
                    {result.executionTime?.toFixed(2)}ms
                </span>
            </div>

            {/* Content Body */}
            <div className="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-terminal-border/50 scrollbar-track-transparent bg-slate-50 dark:bg-terminal-bg">
                {viewMode === 'grid' ? (
                    <table className="w-full text-left border-collapse font-mono text-sm">
                        <thead className="sticky top-0 bg-white dark:bg-terminal-bg z-10 shadow-sm">
                            <tr>
                                {result.columns.map((col, idx) => (
                                    <th key={idx} className="p-3 border-b border-terminal-border text-terminal-text/70 font-bold whitespace-nowrap bg-slate-100 dark:bg-terminal-surface text-xs uppercase tracking-wider border-r border-terminal-border/30 last:border-r-0">
                                        {col}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {result.data.map((row, rowIdx) => (
                                <tr key={rowIdx} className="hover:bg-blue-50/50 dark:hover:bg-white/5 transition-colors group even:bg-white dark:even:bg-white/[0.02] odd:bg-slate-50/50 dark:odd:bg-transparent">
                                    {result.columns.map((col, colIdx) => (
                                        <td key={colIdx} className="p-3 text-terminal-text/90 whitespace-nowrap max-w-xs overflow-hidden text-ellipsis text-xs border-r border-terminal-border/10 last:border-r-0">
                                            {row[col] === null ? (
                                                <span className="text-terminal-text/20 italic text-[10px]">NULL</span>
                                            ) : (
                                                String(row[col])
                                            )}
                                        </td>
